...
function afterEagle(){
  ...
  initSectionFires();
  initCtaCanvas();
  initTypewriter();  /* sets up element + hides cursor — must run BEFORE animateHeroIn */
  animateHeroIn();   /* triggers _startTyping() at the right moment */
  initGSAP();
}
...
function animateHeroIn(){
  ...
  if(typeof gsap!=='undefined'){
    ...
    tl.fromTo(logoWrap,{opacity:0,scale:0.65,filter:'blur(28px)'},{opacity:1,scale:1,filter:'blur(0px)',duration:1.35})
      ...
      .call(()=>{
        if(stats){ stats.classList.add('revealed'); initCounters(stats); }
        /* Start typewriter only after hero is fully revealed */
        const tw=$('#heroTypewriter');
        if(tw && tw._startTyping) tw._startTyping();
      });
  } else {
    ...
    /* Fallback: start typewriter after sub text is visible */
    setTimeout(()=>{ const tw=$('#heroTypewriter'); if(tw&&tw._startTyping) tw._startTyping(); },1800);
  }
}
...
function initTypewriter(){
  ...
  let mi=0,ci=0,del=false,timer=null,started=false;

  /* Hide the cursor blinking until we actually start typing */
  el.style.visibility='hidden';

  function tick(){
    if(!started){
      started=true;
      el.style.visibility='visible'; /* Reveal cursor+text together */
    }
    const msg=msgs[mi];
    ...
    timer=setTimeout(tick,del?45:75);
  }

  /* Don't start until explicitly called after hero-in animation finishes */
  el._startTyping=()=>{ timer=setTimeout(tick,600); };
}
